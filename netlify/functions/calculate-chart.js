...
const constantsPath = require.resolve("../../src/utils/constants.js");
const { zodiacSigns, planets } = require(constantsPath);
...
function getZodiacSign(longitude) {
  ...
  return (
    zodiacSigns.find(
      sign => degree >= sign.startDegree && degree < sign.startDegree + 30
    ) || zodiacSigns[0]
  );
}
  ...
  try {
    ...
    const planetPositions = await Promise.all(
      planets.map(async planet => {
        try {
          const result = await swisseph.swe_calc_ut(
            jd,
            planet.id,
    ...
    // 計算宮位
    const houses = await swisseph.swe_houses(
      jd,
      latitude,
      longitude,
      "P",
    ...
    const fortune = isDayTime
      ? (ascendant.longitude + moonPosition.longitude - sunPosition.longitude) %
        360
      : (ascendant.longitude + sunPosition.longitude - moonPosition.longitude) %
        360;
